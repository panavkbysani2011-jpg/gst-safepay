...
import {
  ...
  deleteImsInvoice,
  type ImsChoice,
  type ImsActionResult,
} from "@/app/ims-actions";
...
export function ImsTable({
  ...
  // Reset the delete arm / message when a different invoice is opened, adjusted
  ...
  function runAction(fn: () => Promise<ImsActionResult>) {
    setActionMsg(null);
          ...
          <>
            ...
            <div className="flex flex-wrap items-center gap-2">
              {(() => {
                ...
                const toggle = (choice: ImsChoice, active: boolean) =>
                  runAction(() => setImsAction(selected.invoiceId, active ? "pending" : choice));
                const base =
                ...
                return (
                  <>
                    <button
                      type="button"
                      disabled={busy}
                      onClick={() => toggle("accept", isAccepted)}
                      aria-pressed={isAccepted}
                      className={`${base} ${isAccepted ? active : idle}`}
                    >
                      {isAccepted && (
                    ...
                    <button
                      type="button"
                      disabled={busy}
                      onClick={() => toggle("reject", isRejected)}
                      aria-pressed={isRejected}
                      className={`${base} ${isRejected ? active : idle}`}
                    >
                      {isRejected && (
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" className="size-4" aria-hidden>
                          <path d="M20 6 9 17l-5-5" />
              ...
              {armedDelete ? (
                <>
                  <button
                    type="button"
                    onClick={() => runAction(() => deleteImsInvoice(selected.invoiceId))}
                    disabled={busy}
