  ...
  for (const v of rows) {
    ...
    if (v.udyamRegistered && v.udyamCategory === null) {
      out.push({
        id: v.id,
        label: v.name,
        message: "Marked MSME-registered but has no category — the 45-day rule needs micro/small/medium.",
      });
    }
    if (dupes.has(v.id)) {
      out.push({ id: v.id, label: v.name, message: `Duplicate id "${v.id}" in this file — only the last row is kept.` });
    }
  }
  return out;
}
...
export function checkBillsHealth(rows: BillInput[], asOf: string): HealthWarning[] {
  ...
  for (const b of rows) {
    ...
    if (dupes.has(b.id)) {
      out.push({ id: b.id, label: b.id, message: `Duplicate id "${b.id}" in this file — only the last row is kept.` });
    }
  }
  return out;
}
...
export function checkImsHealth(rows: ImsInvoiceInput[]): HealthWarning[] {
  ...
  for (const inv of rows) {
    ...
    if (inv.gstAmount > inv.taxableValue) {
      out.push({ id: inv.id, label: inv.invoiceNo, message: "GST amount exceeds the taxable value — check the columns aren't swapped." });
    }
    if (dupes.has(inv.id)) {
      out.push({ id: inv.id, label: inv.invoiceNo, message: `Duplicate id "${inv.id}" in this file — only the last row is kept.` });
    }
  }
  return out;
}
...
export function checkRcmHealth(rows: RcmPurchaseInput[], asOf: string): HealthWarning[] {
  ...
  for (const p of rows) {
    ...
    if (dupes.has(p.id)) {
      out.push({ id: p.id, label: p.vendorName, message: `Duplicate id "${p.id}" in this file — only the last row is kept.` });
    }
  }
  return out;
}
...
export function checkComplianceHealth(rows: ComplianceDeadlineInput[], asOf: string): HealthWarning[] {
  ...
  for (const d of rows) {
    ...
    if (dupes.has(d.id)) {
      out.push({ id: d.id, label: d.name, message: `Duplicate id "${d.id}" in this file — only the last row is kept.` });
    }
  }
  return out;
}
